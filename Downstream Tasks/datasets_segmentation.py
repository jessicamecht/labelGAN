...
def get_datasets(train_size, use_augmentation = False):
    ...
    # file containing shuffled file paths for train/test/val sets
    with open('paths.pkl', 'rb') as f:
        paths = pickle.load(f)
    
    train_dataset =  ChestXrayDatasetSegmentation(root_dir, paths['train_image_paths_original'][0:train_size],
                                                  paths['train_label_paths_original'][0:train_size]) 
    valid_dataset =  ChestXrayDatasetSegmentation(root_dir, paths['valid_image_paths'],
                                                  paths['valid_label_paths'])   
    test_dataset =  ChestXrayDatasetSegmentation(root_dir, paths['test_image_paths'],
                                                 paths['test_label_paths'],
                                                 use_aug=False)  
    
    if use_augmentation:
        augmentation_dataset = AugmentationDatasetSegmentation(root_dir, paths['train_image_paths_synthetic'][0:train_size],
                                                               paths['train_label_paths_synthetic'][0:train_size]) 
